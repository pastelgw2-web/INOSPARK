...
const Navigation: React.FC<NavigationProps> = ({ user, currentView, settings, onNavigate, onLogout }) => {
  ...
  const mainMenus = [
    ...
    { id: 'corporate', label: 'Collaborate', icon: (
    ...
    { id: 'dashboard', label: 'News & Impact', icon: (
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="flex items-center gap-2">
                ...
                >
                  <img src={user.avatar} className="w-7 h-7 md:w-8 md:h-8 rounded-full border border-white shadow-sm" alt={user.name} />
                  <span className="text-[10px] md:text-xs font-black text-slate-700 hidden sm:block">
                    {user.name.split(' ')[0]}
                  </span>
                </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[110] lg:hidden bg-white border-t border-slate-100 h-16 px-6 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <div className="flex justify-between items-center h-full">
          {mainMenus.map(item => (
            ...
            >
              <div className={`transition-all duration-300 ${currentView === item.id ? 'scale-110' : ''}`}>
                {item.icon}
              </div>
