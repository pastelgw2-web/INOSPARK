...
import { InnovationChallenge, User, UserRole } from '../types';
...
const CollaborationHub: React.FC<CollaborationHubProps> = ({ challenges, user, onLoginRedirect, onRoleUpdate }) => {
  const [showProposalModal, setShowProposalModal] = useState<string | null>(null);
  const [showPortalForm, setShowPortalForm] = useState(false);
  const [proposalText, setProposalText] = useState('');

  // Collaborator Guidance & SLA Data
  const guidanceItems = [
    {
      title: "Technical Mentorship",
      desc: "Membimbing inovator dalam standarisasi produk agar layak masuk ke rantai pasok industri.",
      icon: "⚙️"
    },
    {
      title: "Resource Integration",
      desc: "Menyediakan akses ke laboratorium, data teknis, atau komponen khusus untuk percepatan prototipe.",
      icon: "🏢"
    },
    {
      title: "Market Access",
      desc: "Menjadi 'First Buyer' atau mitra distribusi bagi inovasi yang telah lulus uji validasi industri.",
      icon: "🌎"
    }
  ];

  const slaPartner = [
    { label: "Response Time", value: "Max 72 Hours", desc: "Komitmen membalas proposal integrasi dari inovator." },
    { label: "IP Protection", value: "NDA Guaranteed", desc: "Perlindungan hukum penuh atas kekayaan intelektual inovator." },
    { label: "Quarterly Review", value: "Milestone Sync", desc: "Sinkronisasi progres berkala setiap kuartal." }
  ];
  ...
  return (
    <div className="animate-in fade-in duration-700 pb-24">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-16 pt-10">
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter mb-2 leading-none">
            Collaborator <span className="text-emerald-600">Guidance Hub.</span>
          </h1>
          <p className="text-slate-500 font-medium text-lg max-w-2xl leading-relaxed">
            Pemimpin industri dan lembaga riset memberdayakan inovator melalui integrasi teknis dan akses pasar.
          </p>
        </div>
      ...
      {showPortalForm && isCollaborator && (
        <div className="mb-16 animate-in slide-in-from-top-4 duration-500">
          <div className="bg-white p-8 md:p-12 rounded-[2.5rem] border-2 border-emerald-500/20 shadow-2xl shadow-emerald-500/5">
            <h2 className="text-2xl font-black text-slate-900 mb-8">Create New Support Program</h2>
            <form onSubmit={handleCreatePortal} className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <section id="become-partner" className="pt-20 border-t border-slate-100">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight mb-4">Partner With Us</h2>
          <p className="text-slate-500 font-medium text-lg max-w-2xl mx-auto">
            Pelajari peran dan standar layanan (SLA) sebagai mitra industri di platform InnoSpark.
          </p>
        </div>
        ...
        <div className="bg-slate-900 rounded-[3rem] p-10 md:p-16 text-white mb-20 relative overflow-hidden shadow-2xl shadow-slate-900/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              {slaPartner.map((item, idx) => (
                <div key={idx} className="space-y-2">
                  <p className="text-emerald-500 font-black text-2xl">{item.value}</p>
